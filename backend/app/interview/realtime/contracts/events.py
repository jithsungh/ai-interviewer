...
class IntentGapEvent(ClientEvent):
    """
    Client signals a detected response gap for intent classification.

    exchange_id: Sequence order correlation from QuestionPayloadEvent.
    question: Current question text.
    previous_answer: Prior answer text (may be empty).
    last_answer: Latest answer text (may be partial).
    gap_ms: Detected gap duration in milliseconds.
    response_time_ms: Elapsed time since question was asked.
    """

    event_type: Literal["intent_gap"] = "intent_gap"
    exchange_id: int
    question: str
    previous_answer: str
    last_answer: str
    gap_ms: int = Field(gt=0)
    response_time_ms: int = Field(gt=0)
...
class IntentDecisionEvent(ServerEvent):
    """Intent classification result with the next action."""

    event_type: Literal["intent_decision"] = "intent_decision"
    exchange_id: int
    intent: str
    confidence: float
    action: Literal["wait", "clarify", "advance"]
    gap_ms: Optional[int] = None


class ClarificationResponseEvent(ServerEvent):
    """Clarification response generated by the interviewer."""

    event_type: Literal["clarification_response"] = "clarification_response"
    exchange_id: int
